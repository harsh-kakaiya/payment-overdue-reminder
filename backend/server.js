...
app.get('/api/invoices', async (req, res) => {
  ...
  const invoices = [{ id: 1, amount: 100, dueDate: '2023-12-31', recipient: 'John' },{ id: 2, amount: 100, dueDate: '2023-12-31', recipient: 'Kevin' }];
  res.json(invoices);
});
...
const updateGoogleSpreadsheet = async (invoices) => {
  ...
  const auth = new google.auth.GoogleAuth({
    keyFile: 'C:\Users\harsh\Documents\payment-overdue-reminder\backend\package.json', // Path to the downloaded JSON file
...
app.post('/api/trigger-zapier', async (req, res) => {
    try {
      ...
      const dueInvoices = [{ amount: 100, dueDate: '2023-12-31', recipient: 'John' }];
